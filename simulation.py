...
import mock_traffic_controller as controller
import subprocess

def run_vehicle_detection():
    try:
        subprocess.run([sys.executable, "vehicle_detection.py"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"Vehicle detection script exited with error: {e}")
...
activeLane = 0  # Current green lane
timer = 20  # 20 Seconds default timer

defaultMinimum = 10  # Min GST
defaultMaximum = 50  # Max GST
defaultStarvation = 140  # Lane starvation
...
selectedLane = 0  # Select lane for random vehicle input
...
def animate(i):
    data = {'Lane 1': density[0], 'Lane 2': density[1], 'Lane 3': density[2],
            'Lane 4': density[3]}
    keys = list(data.keys())
...
class Main:
    ...
    t4 = Thread(target=run_vehicle_detection)
    t1.setDaemon(True)
    t2.setDaemon(True)
    t3.setDaemon(True)
    t1.start()
    ...
    while True:
        ...
        # Vehicle count of each lane
       # Vehicle count and lane names for each lane
        for i in range(0, 4):
    # Vehicle count
            vehicle_count_text = pygame.font.SysFont(None, 60).render(
                str(lane[i]), True, (0, 0, 0), (255, 255, 255))
            screen.blit(vehicle_count_text,
                        (vehicleCountCoords[i][0], vehicleCountCoords[i][1]))

        # Lane name
            lane_names = ["Lane 1", "Lane 2", "Lane 3", "Lane 4"]
            lane_name_text = font.render(
                lane_names[i], True, (255, 255, 255))  # Black text
            screen.blit(lane_name_text, (vehicleCountCoords[i][0],
                                        vehicleCountCoords[i][1] + 50))  # Adjust position to be right of the vehicle count
        ...
        for i in range(0, 4):
            if (i == activeLane):
                signalTexts[i] = font.render(
                    str(timer), True, (0, 255, 0))
                screen.blit(signalTexts[i], signalTimerCoords[i])

        # Starvation
        timerText = font.render("Starvation: " +
                                str(starvation), True, (255, 255, 255))
        screen.blit(timerText, (50, 50))

        # Simulation runtime
        timerText = font.render("Simulation Time: " +
                                str(simulationTime)+"s", True, (255, 255, 255))
        screen.blit(timerText, (background.get_width() -
                    250, 50))
